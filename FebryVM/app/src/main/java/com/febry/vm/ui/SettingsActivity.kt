...
import com.febry.vm.AppConfig.VPN
import com.febry.vm.R
import com.febry.vm.handler.MmkvManager
import com.febry.vm.handler.SubscriptionUpdater
import com.febry.vm.helper.MmkvPreferenceDataStore
import com.febry.vm.util.Utils
...
class SettingsActivity : BaseActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        ...
        private val themeColor by lazy { findPreference<ListPreference>(AppConfig.PREF_THEME_COLOR) }
        private val scheduledSubUpdate by lazy { findPreference<CheckBoxPreference>(AppConfig.PREF_SCHEDULED_SUB_UPDATE) }
        private val scheduledSubInterval by lazy { findPreference<EditTextPreference>(AppConfig.PREF_SCHEDULED_SUB_UPDATE_INTERVAL) }
        private val dohEnabled by lazy { findPreference<CheckBoxPreference>(AppConfig.PREF_DNS_OVER_HTTPS) }
        private val dohUrl by lazy { findPreference<EditTextPreference>(AppConfig.PREF_DNS_OVER_HTTPS_URL) }
        ...
        override fun onCreatePreferences(bundle: Bundle?, s: String?) {
            ...
            themeColor?.setOnPreferenceChangeListener { _, _ ->
                activity?.recreate()
                true
            }

            scheduledSubUpdate?.setOnPreferenceChangeListener { _, _ ->
                SubscriptionUpdater.syncGlobalSchedule(requireContext())
                true
            }
            scheduledSubInterval?.setOnPreferenceChangeListener { _, _ ->
                SubscriptionUpdater.syncGlobalSchedule(requireContext())
                true
            }

            dohEnabled?.setOnPreferenceChangeListener { _, newValue ->
                dohUrl?.isEnabled = newValue as Boolean
                true
            }
        ...
        override fun onStart() {
            ...
            dohUrl?.isEnabled = MmkvManager.decodeSettingsBool(AppConfig.PREF_DNS_OVER_HTTPS, false)
        }
