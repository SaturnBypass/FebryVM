...
import com.febry.vm.AppConfig.PREF_IS_BOOTED
import com.febry.vm.AppConfig.PREF_ROUTING_RULESET
import com.febry.vm.dto.entities.AssetUrlCache
import com.febry.vm.dto.entities.AssetUrlItem
import com.febry.vm.dto.entities.ProfileItem
import com.febry.vm.dto.entities.RulesetItem
import com.febry.vm.dto.entities.ServerAffiliationInfo
import com.febry.vm.dto.entities.SubscriptionCache
import com.febry.vm.dto.entities.SubscriptionItem
import com.febry.vm.dto.entities.WebDavConfig
...
object MmkvManager {
    ...
    private fun trafficUpKey(guid: String) = "traffic_up_$guid"
    private fun trafficDownKey(guid: String) = "traffic_down_$guid"

    fun addTrafficUpload(guid: String, bytes: Long) {
        val cur = mainStorage.decodeLong(trafficUpKey(guid), 0L)
        mainStorage.encode(trafficUpKey(guid), cur + bytes)
    }

    fun addTrafficDownload(guid: String, bytes: Long) {
        val cur = mainStorage.decodeLong(trafficDownKey(guid), 0L)
        mainStorage.encode(trafficDownKey(guid), cur + bytes)
    }

    fun decodeTrafficUpload(guid: String): Long = mainStorage.decodeLong(trafficUpKey(guid), 0L)
