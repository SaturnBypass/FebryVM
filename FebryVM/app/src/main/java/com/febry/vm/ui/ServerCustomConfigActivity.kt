...
import com.febry.vm.databinding.ActivityServerCustomConfigBinding
...
class ServerCustomConfigActivity : BaseActivity() {
    ...
    private fun saveServer(): Boolean {
        ...
        val rawContent = binding.editor.text.toString()

        // Config Validator: validate JSON syntax first
        if (rawContent.isBlank()) {
            toast(R.string.toast_config_file_invalid)
            return false
        }
        try {
            org.json.JSONObject(rawContent)
        } catch (e: org.json.JSONException) {
            toast(getString(R.string.toast_config_invalid, e.message))
            return false
        }

        val profileItem = try {
            CustomFmt.parse(rawContent)
        } catch (e: Exception) {
